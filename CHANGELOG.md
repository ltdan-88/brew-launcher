...
- Every "here's how to get tmux" message (`--diagnose`, F1 help, the F9
  Launch Preset and Create Preset fallbacks) hardcoded `brew install
  tmux`, even on the pacman backend where that command doesn't work.
  Found while packaging the pacman backend for the AUR. Now says
  `sudo pacman -S tmux` on that backend, unchanged on Homebrew.
